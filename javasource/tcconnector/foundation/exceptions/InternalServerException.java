...
/**
 * Exception returned from the Teamcenter server for any non-service related error.
 * This includes (but is not limited to) communication errors, parsing errors, configuration errors.
 */
public class InternalServerException extends BaseServiceException
{
	 ...
	 * The JSON string returned from the Teamcenter server has one or more messages (each with a code, level, and message).
	 * The array of messages are in reverse order of importance (the low level error that occurred first on the Teamcenter sever
	...
	private static JSONArray getMessages(JSONObject jsonObj)
	{
		...
		String message = "The JSON "+Constants.KEY_QNAME+" value is incorrect for an InternlaServerExcpetion ("+qName+").";
	...
	private static String getPrimaryMessage(JSONArray messages )
	{
		...
		return !primary.isNull(KEY_MESSAGE)? primary.getString(KEY_MESSAGE): Integer.toString(primary.getInt(KEY_CODE));
