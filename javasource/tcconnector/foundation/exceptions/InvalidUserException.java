 ...
 * This indicates that the client application has not authenticated the user yet, or 
 * the authenticated session for that user has time-out.
 */
...

{
	...
	private static String getMessage(JSONObject jsonObj )
	{
		...
		if(qName.equals(Constants.QNAME_USER_EXP))
			return !jsonObj.isNull(KEY_MESSAGE)? jsonObj.getString(KEY_MESSAGE): Integer.toString(jsonObj.getInt(KEY_CODE));
