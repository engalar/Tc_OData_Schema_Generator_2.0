...
/**
 * The default mapping is used to map the Teamcenter structure element (keys in JSONObject) names
 * to the Entity member (Attributes or Associations) names.
 * The names must match one-to-one with the following caveats:
 * <ul>
 *  <li> The Entity member name is prefixed with a '_', i.e. '_type'.
 *     In this case the '_' is ignored, thus matching the Teamcenter name of 'type'. </lie>
 *  <li> The Entity member name is suffixed with '__XXX', i.e. 'phone__Home'.
 *     In this case the '__Home' is ignored, thus matching the Teamcenter name of 'phone'. </li>
 *</ul>
 *     
 * This mapping may be applied to Teamcenter service operation data structures, and the equivalent Mendix Entity objects.
 * The Entity names are not used, it is only the Associations between Entities that must match (or be mappable).
 */
