...
public class TcConnection 
{
	...
	static
	{
		...
		NOT_LOGGED_IN.put(InvalidUserException.KEY_MESSAGE, "Please login to Teamcenter before calling other service operations.");	
	}
	 ...
	 * @param serviceName	The name of the service/operation (Core-2011-06-Session/login)
	 * @param jsonInputObj  The input arguments of the service operation.
	 * @param jsonPropPolicyObj The Object Property Policy for the service operation.
	 * @param tcSession 
	 * @return				The returned data structure of the service operation.
	 * 						All business object references (UIDs) throughout the response are replaced with the full
	 * 						ModelObject instance from the ServiceData. The JSONObject for these instance may be cast
	 * 						to JModelObject, which offers convenience methods for accessing property values.
	 * @throws Exception    Any non-service operation error encountered while processing the service requests are 
	 * 						caught by this method and displayed in a pop-up message dialog. The exception is re-thrown here.
	 */
	public static JSONObject callTeamcenterService( IContext context, String serviceName,
													JSONObject jsonInputObj, JSONObject jsonPropPolicyObj, String configurationName ) throws Exception 
	{
		TcConnection connection = new TcConnection(context, serviceName, jsonInputObj, jsonPropPolicyObj,configurationName);
	 ...
	 * @param responseObject   			The returned object. If null an instance of ServiceResponse will be created for the return.
	 * @param operationMapping 			Path (relative to resources/OperationMappings) for the operation mapping definition.
	 *                         			The inputArgument and responseObject are mapped to the service operation input/output based on 
	 *                         			the definition in the mapping file.
	 * @param businessObjectMappings	A semicolon separated list of Teamcenter business object names and Entity names 
	 ...
	 * 						 caught by this method and displayed in a pop-up message dialog. The exception is re-thrown here.
	 */
	public static ServiceResponse callTeamcenterService( IContext context,String serviceName, IMendixObject inputArgument, ServiceResponse responseObject, 
			                                    String operationMapping, String businessObjectMappings,String configurationName ) throws Exception 
	{
		...
		ServiceMapper serviceMapper= new ServiceMapper(context, serviceName,  operationMapping, businessObjectMappings,configurationName);
		...
		JSONObject jsonResponseObj = TcConnection.callTeamcenterService(context, serviceName, jsonInputObj, jsonPolicy,configurationName);
	 ...
	 */
	public static JSONObject callTeamcenterService( IContext context, String serviceName, 
													String jsonInput, JSONObject jsonPropPolicyObj,String configurationName ) throws Exception 
	{
	...
	private TcConnection(IContext context, String serviceName, JSONObject jsonInputObj, JSONObject jsonPropPolicyObj, String configurationName)
	{
	...
	private JSONObject sendRequest() throws Exception 
	{
		try
		{
			...
			ct.startTransaction();
            tcSession = tcconnector.proxies.microflows.Microflows.retrieveTcSessionBasedOnConfigName(ct, configurationName);
            ct.endTransaction();
            
			hostAddress = tcSession.getHostAddress();
			
			if(hostAddress == null) // Host Address is set in ShowLoginPage microflow, if null, user has not logged in yet
			...
			HttpResponse httpResponse = tcconnector.proxies.microflows.Microflows.sendService(context,  soaServiceRequest);
	...
	private void logRequest ( )
	{
		if(Constants.LOGGER.isTraceEnabled() &&
		   !serviceName.endsWith("login")    &&
		   !serviceName.endsWith("loginSSO"))
			Constants.LOGGER.trace( LogCorrelationID.getId()+": Service Request  - "+serviceName+"\n"+jsonInputObj.toString());
	...
	private void logResponse( long duration, int bytesOut, String jsonResponseDoc )
	{
		...
		String text =  String.format("%-120s (Time:%9dms, Request:%9dbytes, Response:%9dbytes)", 
						idAndName,duration, bytesOut,jsonResponseDoc.length() );
	...
	private void  pushLogCorrelationIDRequestCount( TcSession tcSession ) throws Exception
	{
		synchronized( COUNT_LOCK )
		{
			...
			// In Mendix, tcSession.commit doesn't actually do commit to DB. In Mendix DB commit seems 
			// to happen after a Microflow is complete. 
			// So we need to explicitely do a commit and release the lock from object from DB.
			// 
			// If there are more than one threads call SOA, then first thread who comes in synchronized
			// block does not finish the block and release the COUNT_LOCK object.
			// It continue to hold lock on tcSession object in DB due to recursive microflow & mutliple
			// SOA calls in connector. 
			// This does not happen if SOAs are called sequentially.
			try {
					/*IContext ct =  context.getSession().createContext();
		            ct.startTransaction();
		            tcSession.commit(ct); 
		            ct.endTransaction();*/
				    tcSession.commit(); 
	...
	private void logServiceExcpetion(BaseServiceException se) throws BaseServiceException
	{
		...
		while(se.getCause() != null)
		{
			...
			logMessage += "\nCaused by - "+se.getClass().getSimpleName()+" ("+se.getCode()+"): "+se.getMessage();
		}
	...
	private void logAnyExcpetion( Exception e ) throws Exception
	{
		...
		String logMessage = message +  " "+ e.getClass().getSimpleName()+": "+e.getMessage(); // This message is from the REST action and is not helpful to an end user
