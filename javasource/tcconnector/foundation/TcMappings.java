...
/**
 * Defines mappings between Teamcenter structure element (keys in JSONObject) names
...
public abstract class TcMappings 
{
	...
	/** @return  true if the JSONObject represents a reference to a ModelObject (it has the keys 'uid', 'className', and 'type').*/
	public boolean isAModelObjectRef( JSONObject jsonObj )
	{
		if( jsonObj.length() < 3) return false;
		if(!jsonObj.has(JModelObject.UID)        || jsonObj.isNull(JModelObject.UID) ) 		  return false;
		if(!jsonObj.has(JModelObject.CLASS_NAME) || jsonObj.isNull(JModelObject.CLASS_NAME) ) return false;
		if(!jsonObj.has(JModelObject.TYPE)       || jsonObj.isNull(JModelObject.TYPE) ) 	  return false;
		return true;
	}
