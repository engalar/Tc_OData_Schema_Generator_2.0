...
public class JPolicy extends JSONObject
{
	...
	private static final String UI_ONLY            = "uIValueOnly";
	...
	private JSONObject createType(String typeName, IMetaObject meta, Set<TcMapping> objMapping, boolean withProperties)
	{
	...
	private JSONObject createPropertyFromAttribute( String propName, IMetaObject meta, String memberName)
	{
		...
		if(attributeType.getType().equals(PrimitiveType.String) &&  !memberName.endsWith(DB_SUFFIX))
			jProp.getJSONArray(KEY_MODIFIERS).put(createModifier(UI_ONLY));
		else
			jProp.getJSONArray(KEY_MODIFIERS).put(createModifier(EXCLUDE_UI));
		return jProp;
	}
	
	private JSONObject createPropertyFromAssociation( String propName, IMetaObject meta, String memberName, boolean withProperties)
	{
