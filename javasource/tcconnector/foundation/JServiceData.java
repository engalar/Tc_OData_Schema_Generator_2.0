...
public class JServiceData extends JSONObject
{
	...
	static
	{
		...
		Iterator<String> keys = Arrays.asList(new String[] {PLAIN, UPDATED, CREATED, DELETED, PARTIAL_ERRORS, MODEL_OBJECTS, Constants.KEY_QNAME}).iterator();
	 ...
	 * @throws IllegalArgumentException	The JSONObject does not have the required keys/fields that represent a ServiceData.
	 */
	 ...
	 * 
	 */
	public IMendixObject instantiateEntity( IContext context, BusinessObjectMappings boMappings, String configurationName )
	{
	 ...
	 * 
	 */
	public ServiceData instantiateServiceData( IContext context, BusinessObjectMappings boMappings,String configurationName )
	{
	...
	private void instantiateList( IContext context, String key, ModelObject.MemberNames memberName, ServiceData tgtSD, BusinessObjectMappings boMappings, String configurationName)
	{
		for(JModelObject jModelObj: getNamedList(key))
		{
			IMendixObject plainObj = jModelObj.instantiateEntity(context, ModelObject.entityName, boMappings,configurationName);
		...
		for(JErrorStack jError :getPartialErrors() )
		{
			for(int cnt = 0 ; cnt < jError.length(); ++cnt)
			{
				...
				for(int errorValuesCnt = 0 ; errorValuesCnt < errorValues.length(); ++errorValuesCnt)
				{
					...
					errorStack.getMendixObject().setValue(context, ErrorStack.MemberNames.PartialErrors.toString(), tgtSD.getMendixObject().getId());
				}
			}
		}
	}
