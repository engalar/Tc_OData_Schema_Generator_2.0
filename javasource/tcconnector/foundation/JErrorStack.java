...
/**
 *  A single error returned by a service operation.
 *  Since most service operations are set based, the operation may return a different
 *  JErrorStack for each element in the set that failed.
 *  The JErrorStack may also have an optional UID, Client ID, or Index to help identify
 *  the source element  that caused this error. 
 */
	...
	private void validateErrorKeys( JSONArray array )
	{
		...
		for(int i=0; i<primaryMessageIndex; i++)
		{
			...
			while(it.hasNext())
			{
				...
				{
					Constants.LOGGER.error(LogCorrelationID.getId()+": The JSONObject does not represent a ErrorValue.\n"+value.toString());
