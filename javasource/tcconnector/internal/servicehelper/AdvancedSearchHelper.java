...
public class AdvancedSearchHelper 
{
	...
	/**
	 * performs the advance search operation based on query input
	 * reagrding 
	 * @param targetObject	The service operation  response, may be a ServiceData or custom data structure.
	 * @throws Exception 
	 */
	public static IMendixObject performAdvanceSearch( IContext context, String queryName, IMendixObject specialisedSearchInput, String businessObjectMapping, String configurationName ) throws Exception
	{
		...
		//Set queryUid value from findSavedQuery SOA to 'queryUID' attribute of Saved Query
		IMendixObject searchCriteriaInput = Core.retrieveByPath(context, specialisedSearchInput, KEY_SEARCH_CRITERIA).get(0);
		...
		CallTeamcenterService performSearch = new CallTeamcenterService(context, Constants.OPERATION_PERFORMSEARCH, specialisedSearchInput, searchResponse, KEY_SEARCH_MAPPING, businessObjectMapping, configurationName);
		return performSearch.executeAction();
	}
	
	public static String getSavedQueryUID( IContext context, String queryName, String ConfigurationName) throws Exception
	{
		...
		CallTeamcenterService findSavedQuery = new CallTeamcenterService(context, Constants.OPERATION_FINDSAVEDQUERIES, 
												findSavedQueryInput.getMendixObject(), queryResponse, KEY_OPERATION_MAPPING, KEY_SAVED_QUERY_OBJECT_MAPPING, ConfigurationName);
