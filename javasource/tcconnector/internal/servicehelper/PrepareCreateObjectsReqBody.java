...
public class PrepareCreateObjectsReqBody
{
	 ...
	 * @throws InvalidInputException	 If the input __referencePropName property is empty.
	 */	
	...
	public java.lang.String getCreateDataJsonStr(IMendixObject createInput, List<Short> listOfProcessedEntities){
		...
		try {
			...
			if(boName.isEmpty())
			{
				String message = "Property "+ createInput.getMetaObject().getName() + "."+ ENTITY_BONAME_PROP_KEY + "is empty.";
	    	...
	    	for(int referenceIndex = 0; referenceIndex < references.size(); ++referenceIndex ) {
	            ...
	            if(identifier != null) {
		            ...
		            if(associationName.contains( REQ_COMPOUND_CREATE_INPUT_REF_MEMBER_PROP_KEY ) && (referencedInput != null)) {
			            ...
			            if(!listOfProcessedEntities.contains(entityId)) 
			            {
			    			...
			    			{
			    				String message = "Property "+ createInput.getMetaObject().getName() + "."+ "compoundCreateInput.CompndCreateInput." + ENTITY_REF_PROP_KEY + "is empty.";
			    				throw new InvalidInputException(message);
			    			}			            	
