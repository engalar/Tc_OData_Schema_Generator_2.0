...
public class OperationMappingCache 
{
	...
	private static final String[] VALID_KEYS = new String[]{OperationMapper.KEY_SERVICE_OPERATION, OperationMapper.KEY_INPUT_TYPE, 
															OperationMapper.KEY_RESPONSE_TYPE,     OperationMapper.KEY_OBJECT_MAPPING,
															OperationMapper.KEY_OPERATION_INPUT,   OperationMapper.KEY_OPERATION_RESPONSE};

	/**
	 * Get the cached mapping template 
	 * @param mappingSource		A file path relative to resources/OperationMapping folder, or an actual JSON string
	 * @return
	 */
	...
	private InputStream getInputStream( String mappingFile )throws IOException
	{
		...
		throw new IOException("The file does exist in the Resource folder ("+
							  mapFile.getAbsolutePath()+") or in the ClassPath ("+mappingFile+").");
	}
	...
	public void validateKeys( JSONObject  operationMapObj)
	{
		for(String key : VALID_KEYS)
		{
			...
			{
				Constants.LOGGER.error(LogCorrelationID.getId()+": The operation mapping is missing the required key '"+key+"'.");
