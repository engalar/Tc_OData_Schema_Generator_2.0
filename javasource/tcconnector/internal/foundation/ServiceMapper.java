...
public class ServiceMapper 
{
	...
	public ServiceMapper(IContext context, String serviceName, String operationMapping, String businessObjectMapping,String configurationName ) throws Exception
	{
	...
	/**
	 * @return The Object Property Policy for this service operation (as defined by the BusinessObjectMappings).
	 */
	...
	/**
	 * Maps the source Mendix object to a JSONObject as defined by the operationMapping.
	 * @param inputArgument		The source input argument for the service operation.
	 * @return					The JSONObject that can be used as input to the TcConnection.callTeamcenterService
	...
	public JSONObject mapInputData(IMendixObject inputArgument ) throws Exception
	{
		...
		{
			String message = "Failed to map the input Entity to JSON for the "+serviceName+" service. "+e.getMessage();
	...
	/**
	 * Maps the source JSONObject to the Mendix object as defined by the operationMapping.
	 * @param jsonResponseObj	The source JSONObject returned from TcConnection.callTeamcenterService.
	...
	public ServiceResponse mapOutputData( JSONObject jsonResponseObj, ServiceResponse responseObject ) throws Exception
	{
		...
		try
		{
			Constants.LOGGER.trace("Mapping the output data");
			OperationMapper.toDomainModelEntity(context, jsonResponseObj, responseObject.getMendixObject(), operationMapping, boMappings, configurationName );
			autoMapServiceData(jsonResponseObj,  responseObject, configurationName);
			return responseObject;
		}
		catch(Exception e)
		{
			String message = "Failed to map the response JSON to return Entity object for the "+serviceName+" service. "+e.getMessage();
	...
	private void validatArguments( String businessObjectMapping, String configurationName) throws Exception
	{
		try
		{
			if(serviceName == null || !serviceName.contains("/"))
				throw new IllegalArgumentException("The ServiceName argument ("+serviceName+") must have the syntax <service name>/<operation name> "+
		...
		{
			String message = "The input arguments for  the "+serviceName+" service operation call are not valid. "+e.getMessage();
			Constants.LOGGER.error(message);
			throw e;			
		}		
	}
	
	private void autoMapServiceData(JSONObject jsonResponseObj, ServiceResponse responseObject,String configurationName) throws Exception
	{
		if(responseObject.getResponseData() != null)
			return;
		
		JServiceData jSD = (JServiceData)ModelObjectResolver.getServiceData(context, jsonResponseObj ,configurationName );
		if(jSD == null)
			return;
