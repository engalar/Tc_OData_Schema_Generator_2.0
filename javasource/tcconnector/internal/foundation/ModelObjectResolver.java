...
public class ModelObjectResolver 
{
	...
	/**
	 * Replace all ModelObject references (UIDs) with the actual ModelObject instance from
	 * the ServiceData.modelObjects map.
	 * 
	 * @param responseObj	The service operation  response, may be a ServiceData or custom data structure.
	 */
	public static JSONObject resolve( IContext context, JSONObject responseObj,String configurationName )
	{
		ModelObjectResolver resolver = new ModelObjectResolver(context, responseObj, TcModelObjectMappings.INSTANCE,configurationName);
	...
	private JSONObject findServiceData()
	{
		...
		if( rootObject.has(Constants.KEY_QNAME)    && 
		   !rootObject.isNull(Constants.KEY_QNAME) &&
	...
	{
		if( responseObj.has(KEY_MODEL_OBJECTS)    && 
		   !responseObj.isNull(KEY_MODEL_OBJECTS))
		{
	...
	{
		if(uid.equals(JModelObject.NULL_TAG_UID) || 
		   !modelObjects.has(uid))
	...
	private void resolveChildObject( JSONObject parentStruct, String key)
	{
		...
		if(tcMappings.isAModelObjectRef(childStruct))
		{
			...
			if(mo==JSONObject.NULL)
			{
				...
				parentStruct.put(key, childStruct);
			}
			else
			{
				parentStruct.remove(key);
	...
	{	
		...
		if(! ((firsElement instanceof JSONObject) ||
			  (firsElement instanceof JSONArray)))
			return tgtArray;
	...
	private void parseProperty(JSONObject props, String name)
	{
		...
		if(!property.has(JModelObject.DB_VALUES) || 
				!(property.getJSONArray(JModelObject.DB_VALUES).length() > 0))
			return;
		...
		if(!(modelObjects.has(possibleUID) ||
		    possibleUID.equals(JModelObject.NULL_TAG_UID)))
			return;
