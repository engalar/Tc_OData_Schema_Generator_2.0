...
/**
 * Maps data between Teamcenter defined data structures (service operation input and output data types), and Mendix Entities.
 *
 */
public class TcDataMapper 
{
	...
	/**
	 * Maps a Teamcenter data structure (service operation response object) to the Mendix Entity.
	 * The {@link TcDefaultMappings} is used to map the Teamcenter structure element (keys in JSONObject) names
	...
	public static void toDomainModelEntity(IContext context, JSONObject srcJsonObj, IMendixObject tgtEntity, BusinessObjectMappings boMappings,String configurationName )  
	{
		TcDataMapper dm = new TcDataMapper(context, TcDefaultMappings.INSTANCE, boMappings,configurationName);
		dm.toEntity(srcJsonObj, tgtEntity);
	}	
	
	public static void setDomainModelMemberValue(IContext context, IMendixObject tgtEntity, String memberName, Object srcValue, BusinessObjectMappings boMappings,String configurationName  )  
	{
		TcDataMapper dm = new TcDataMapper(context, TcDefaultMappings.INSTANCE, boMappings,configurationName);
	...
	/**
	 * Maps a Mendix Entity to the Teamcenter data structure (service operation input object).
	 * The {@link TcDefaultMappings} is used to map the Entity member (Attributes or Associations) names 
	 * to the Teamcenter structure element (keys in JSONObject) names. 
	 * 
	 * @param context		The Request context.
	...
	public static Object toJSONValue( IContext context, List<IMendixObject> srcEntity, String memberName, Map<String,Object> instructions)
	{
		TcDataMapper dm = new TcDataMapper(context, TcDefaultMappings.INSTANCE, instructions);
	...
	private Map<String,Object>		instructions = new Hashtable<>();
	...
	private TcDataMapper(IContext context, TcMappings mappings, BusinessObjectMappings boMappings,String configurationName)
	{
		this.context   = context;
		this.mappings  = mappings;
	...
	private void setMemberValue( IMendixObject tgtEntity, String memberName, Object srcJsonValue )
	{
		if(tgtEntity.hasMember(memberName))
		{
			...
			else if (member instanceof MendixObjectReferenceSet)
			{
				...
				{
					setMemberChildMap( tgtEntity, memberName, (MendixObjectReferenceSet)member,  (JSONObject) srcJsonValue );		
				}		
			}
			else
			{
	...
	private void setMemberChildValues( IMendixObject tgtEntity, MendixObjectReferenceSet member,  JSONArray srcJsonArray )
	{
		...
		for(int i = 0; i < srcJsonArray.length(); i++)
		{
			...
			if(jsonElement instanceof JSONObject)
			{
				if(jsonElement instanceof JModelObject)
				{
					...
					IMendixObject elmentEntity = jModelObj.instantiateEntity(context, childType, boMappings,configurationName);					
	...
	{	
		...
		for(int i = 0; i < srcJsonArray.length(); i++)
		{
			...
			if(jsonElement instanceof JModelObject)
			{
				JModelObject jModelObj = (JModelObject)jsonElement;
				IMendixObject elmentEntity = jModelObj.instantiateEntity(context, childType, boMappings,configurationName);					
				((MendixObjectReference)elmentEntity.getMember(context,memberName)).setValue(context, tgtEntity.getId());			
			}
	...
	private void setMemberChildMap( IMendixObject tgtEntity, String memberName, MendixObjectReferenceSet member,  JSONObject srcJsonObj )
	{
	...
	private void setMemberParentMap( IMendixObject tgtEntity, String memberName, JSONObject srcJsonObj )
	{
		...
		while(keys.hasNext())
		{
			...
			((MendixObjectReference)entry.getEntry().getMember(context,memberName)).setValue(context, tgtEntity.getId());			
		}
	}
	...
	private JSONObject toJSON( IMendixObject srcEntity)
	{
		...
		for(TcMapping mapping : jsonToMembers)
		{
			...
			else if(!instructions.containsKey(INSTRUCTION_IGNORE_NULL))
			{
				if(srcEntity.getMetaObject().getMetaPrimitive(memberName) != null)
					tgtJsonObj.put(jsonKey, "");
				else if(srcEntity.getMetaObject().getMetaAssociationParent(memberName) != null)
					if(srcEntity.getMetaObject().getMetaAssociationParent(memberName).getOwner().name() == "BOTH")
						tgtJsonObj.put(jsonKey, "");
					else
						tgtJsonObj.put(jsonKey, new ArrayList<>());
				else if(srcEntity.getMetaObject().getMetaAssociationChild(memberName) != null)
					if(srcEntity.getMetaObject().getMetaAssociationChild(memberName).getOwner().name() == "BOTH")
						tgtJsonObj.put(jsonKey, "");
					else
						tgtJsonObj.put(jsonKey, new ArrayList<>());
			}
	...
	private Object toJSONValue( List<IMendixObject> srcEntity, String memberName )
	{
		...
		for( IMendixObject leaf : srcEntity)
		{
			...
			if( mxValue == null ||
			   (mxValue instanceof IMendixIdentifier && 
			    processed.contains((IMendixIdentifier)mxValue)))
				if(!instructions.containsKey(INSTRUCTION_IGNORE_NULL))
				{
					if(leaf.getMetaObject().getMetaPrimitive(memberName) != null)
						...
						if(leaf.getMetaObject().getMetaAssociationParent(memberName).getOwner().name() == "BOTH")
							return "";
						else
							return new ArrayList<>();
					else if(leaf.getMetaObject().getMetaAssociationChild(memberName) != null)
						if(leaf.getMetaObject().getMetaAssociationChild(memberName).getOwner().name() == "BOTH")
							return "";
						else
							return new ArrayList<>();
				}
	...
	{
		// Date is converted to String, other primitives can be added directly to the JSONObject
		...
		if(tgtValue instanceof Date )
		{
			...
			if(instructions.containsKey(INSTRUCTION_DATE))
				tgtValue = PropertyResolver.serializeDate((Date)tgtValue, (SimpleDateFormat)instructions.get(INSTRUCTION_DATE));
			else
	...
	private Object getMemberParentMap(List<IMendixObject> srcEntries)  
	{
		...
		{
			if(srcEntry == null ||
			   processed.contains(srcEntry.getId()))
				continue;
	...
	private Object getMemberParentArray(List<IMendixObject> srcElements )
	{
		...
		{
			if(srcElement == null ||
			   processed.contains(srcElement.getId()))
				continue;
