 ...
 * The Mapping file contains a JSON template for the given service operation Request and Response with
 * developer defined mappings.
 * Values in the JSON template are either hard coded values, or use a $Input or $Response substitution keyword.
 * 
 ...
 *</pre>
 *
 * The Entity Member (Attribute or Association) names mapped one-to-one with the Teamcenter service operation data structure
 * with the following caveats:
 *<ul>
 * <li>The Entity Member name may be prefixed with a '_', i.e. '_type'.   In this case the '_' is ignored, 
 * 		thus matching the Teamcenter name of 'type'.</li>
 * <li>The Entity Member name may be suffixed with '__XXX', i.e. 'phone__Home'. In this case the '__Home' is ignored, 
 * 		thus matching the Teamcenter name of 'phone'.</li>
 *</ul>
 */
public class OperationMapper 
{
	 ...
	 */	
	public static void toDomainModelEntity(IContext context, JSONObject srcJsonObj, IMendixObject tgtEntity, String mappingFile, BusinessObjectMappings boMappings,String configurationName )  
	{
		if(mappingFile == null)
	...
	public static BusinessObjectMappings getBusinessObjectMappings( String mappingFile, BusinessObjectMappings boMappings)
	{
		if(boMappings == null)
		{
	...
	private OperationMapper( IContext context, String mappingFile, BusinessObjectMappings boMapping,String configurationName)
	{
		this.context         = context;
		this.operationMapObj = OperationMappingCache.getMapping( mappingFile );
		this.replacePrefix   = "";	
	...
	private Object resolveValue( String replacement, IMendixObject srcEntity )
	{
		...
		catch(Exception e)
		{
			...
			throw new IllegalArgumentException("Failed to to resolve the value for '"+replacement+"'. " +e.getMessage());
		}
	}
	...
	private void resolveValue( String replacement, Object srcAny, IMendixObject tgtEntity  )
	{
		...
		{			
			...
			{	
				// Just the $Response was given for the replacement, so tgtEntity and leafParent are the same
				...
				{
					TcDataMapper.toDomainModelEntity(context, (JSONObject)srcAny, leafParentEntity, boMapping,configurationName);
				}
				return;
			}
			TcDataMapper.setDomainModelMemberValue(context, leafParentEntity, memberName, srcAny, boMapping,configurationName);
	...
	private void resolveArrayValue( String replacement, Object srcAny, IMendixObject tgtEntity  )
	{
		...
		{			
			...
			TcDataMapper.setDomainModelMemberValue(context, leafParentEntity, associationName, srcAny, boMapping,configurationName);
		}
		catch(Exception e)
		{
			Constants.LOGGER.error("Failed to to resolve the value for '"+replacement+"'. " +e.getMessage());
