...
public class SSORedirectRequestHandler extends RequestHandler 
{
	...
	@Override
	public void processRequest(IMxRuntimeRequest request, IMxRuntimeResponse response, String path)
			throws Exception {
	...
	private void processCallback(IMxRuntimeRequest request, IMxRuntimeResponse response) throws CoreException, SSOInvalidSessionException, SSOAuthorizationException, SSOSystemException, SSOConfigurationException, SSOException {
		...
		TeamcenterConfiguration config = tcconnector.proxies.microflows.Microflows.retrieveTeamcenterConifgurationByName(context,tcConfigName); 
        ...
        SSOAppSession ssoAppSession = new SSOAppSession( mendixAppID, handle, identityURL );
		...
		// Mendix code to redirect to home page
	...
	private void executeTeamcenterLogin(IContext context, Credentials credentials,TeamcenterConfiguration config) {
		...
		params.put("Credentials", credentials.getMendixObject());
		params.put("TeamcenterConfiguration", config.getMendixObject());
		try {
			Core.execute(context, "TcConnector.ExecuteLogin", params);
		} catch (Exception e) {
